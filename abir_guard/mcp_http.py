...
class AuthValidator:
    ...
    def validate(self, provided_key: Optional[str]) -> bool:
        ...
        if not provided_key:
            return False
        return any(hmac.compare_digest(provided_key, k) for k in self._valid_keys)
    ...
    def do_POST(self):
        ...
        if self.rate_limiter:
            ...
            if not allowed:
                ...
                self.end_headers()
                self.wfile.write(
                    json.dumps({"error": {"code": 429, "message": "Rate limit exceeded"}}).encode()
                )
                return
        ...
        if content_length > self.max_body_size:
            self._log_audit("body_too_large", client_ip, False, f"size={content_length}")
        ...
        if self.server_instance:
            ...
            self._log_audit(
                "mcp_request", client_ip, "error" not in response, request.get("method", "unknown")
            )
        else:
