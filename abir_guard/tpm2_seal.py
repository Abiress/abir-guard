...
class TPM2Sealer:
    ...
    def is_available(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["tpm2_getcap", "properties-fixed"], capture_output=True, text=True, timeout=5
            )
    ...
    def seal(
        ...
        try:
            ...
            # Create PCR policy
            result = subprocess.run(
                ["tpm2_createpolicy", "--policy-pcr", "-l", pcr_policy, "-f", policy_file],
                capture_output=True,
                text=True,
                timeout=10,
            )
            ...
            cmd = [
                "tpm2_create",
                "-C",
                primary_ctx,  # parent key context
                "-i",
                secret_file,
                "-u",
                f"{sealed_file}.pub",
    ...
    def unseal(self, sealed: SealedData, auth_value: Optional[bytes] = None) -> bytes:
        ...
        try:
            # Split sealed blob back to pub/priv (simplified)
            ...
            if result.returncode != 0:
                raise TPM2Error(f"TPM unseal failed (likely PCR mismatch): {result.stderr}")
    ...
    def create_primary_key(self, algorithm: str = "ecc") -> str:
        ...
        cmd = ["tpm2_createprimary", "-C", "o", "-c", ctx_file, "-g", "sha256", "-G", algorithm]
    ...
    def _compute_policy_hash(self, pcr_indices: List[int]) -> str:
        ...
        for idx in pcr_indices:
            try:
                result = subprocess.run(
                    ["tpm2_pcrread", f"sha256:{idx}"], capture_output=True, text=True, timeout=5
                )
                if result.returncode == 0:
