...
class HardwareEnclave:
    ...
    def _check_apple_se(self) -> EnclaveInfo:
        ...
        return EnclaveInfo(
            platform=EnclavePlatform.APPLE_SECURE_ENCLAVE
            if available
            else EnclavePlatform.SOFTWARE,
            available=available,
            details=details,
            warnings=warnings,
        )
        ...
        try:
            ...
            if "sgx" in cpuinfo.lower():
                return EnclaveInfo(
                    ...
                    warnings=[]
                    if self._check_sgx_available()
                    else ["SGX detected but not enabled"],
                )
        except Exception:
            pass
        ...
        try:
            ...
            if "sev" in cpuinfo.lower():
                return EnclaveInfo(
                    ...
                    else ["SEV detected but /dev/sev missing"],
                )
        except Exception:
            pass
    ...
    def _check_sgx_available(self) -> bool:
        ...
        return (
            os.path.exists("/dev/isgx")
            or os.path.exists("/dev/sgx")
            or os.path.exists("/dev/sgx_enclave")
        )
