...
- **Eliminated Ambiguous `None`-Default API (MEDIUM):** `MLKEM1024`, `HybridKem`, and `PostQuantumTls` now take `require_pq: bool = True` — no more `bool | None = None` overloading where `None` silently meant "default to strict." Environment variable `ABIR_GUARD_REQUIRE_PQ` is now read only by the `create_ml_kem()` factory function, not by constructors. This eliminates an entire class of latent bugs where forwarded `None` values silently enabled strict mode.
