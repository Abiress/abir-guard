...
class TestYubiKeyIntegration:
    ...
    def test_get_fido2_info_requires_device(self):
        """CTAP2 info query requires a connected device."""
        yk = self.YubiKeyManager()
        with patch.object(yk, 'is_available', return_value=False):
            with pytest.raises(self.YubiKeyNotFoundError):
                yk.get_fido2_info()

    def test_list_piv_slots_requires_device(self):
        """PIV slot listing requires a connected device."""
        yk = self.YubiKeyManager()
        with patch.object(yk, 'is_available', return_value=False):
            with pytest.raises(self.YubiKeyNotFoundError):
                yk.list_piv_slots()
...
class TestTPM2Seal:
    ...
    def test_backend_mode(self):
        """Backend mode reports one of the supported paths."""
        tpm = self.TPM2Sealer()
        assert tpm.get_backend_mode() in {"native-tss", "cli", "software"}
