    ...
    def test_require_pq_flag_behavior(self):
        ...
        if probe.backend() == "x25519":
            with pytest.raises(SecurityException):
                MLKEM1024(require_pq=True, warn_on_fallback=False)
        else:
            kem = MLKEM1024(require_pq=True, warn_on_fallback=False)
            assert kem.backend() in {"pqcrypto", "liboqs"}

    def test_require_pq_env_behavior(self, monkeypatch):
        monkeypatch.setenv("ABIR_GUARD_REQUIRE_PQ", "true")
