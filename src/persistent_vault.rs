...
pub fn get_vault(passphrase: &str) -> Vault {
    ...
    for key in stored {
        vault
            .import_key(&key.key_id, &key.public_key, &key.secret_key)
            .ok();
    }
...
pub fn persist_mldsa_keys(
    keypairs: &[(String, MldsaKeypair)],
    passphrase: &str,
) -> Result<(), String> {
...
pub fn sign_with_vault(key_id: &str, data: &[u8], passphrase: &str) -> Result<Vec<u8>, String> {
    ...
    crate::ml_dsa::sign(data, &kp.signing_key).map_err(|e| format!("Signing failed: {}", e))
}

/// Verify data with ML-DSA using vault-stored keys
pub fn verify_with_vault(
    key_id: &str,
    data: &[u8],
    signature: &[u8],
    passphrase: &str,
